...
def draw_player_ui(player):
    ...
    screen.blit(hp_text,(40,HEIGHT-230))
    ...
    screen.blit(energy_text,(40,HEIGHT-185))
    ...
    screen.blit(block_text,(40,HEIGHT-155))
...
def run_pygame_game(player,enemies):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if exit_btn.is_clicked(mouse):
                    pygame.quit(); sys.exit()
                if end_turn_btn.is_clicked(mouse):
                    turn_ended = True

                # KLICKA KORT
                if not targeting_enemy:
                    for i,cu in enumerate(card_uis):
                        if cu.rect.collidepoint(mouse):
                            card = player.hand[i]
                            if card.attack>0:
                                selected_card = i
        ...
        if turn_ended:
            ...
            for e in enemies:
                e.choose_intent()
                enemy_attacks.append(EnemyAttackAnimation(e.ui.rect.x,e.ui.rect.y))
                e.act(player)
                e.intent = None
        ...
        if all(e.hp<=0 for e in enemies):
            ...
            text = big_font.render("VICTORY!",True,(255,255,0))
